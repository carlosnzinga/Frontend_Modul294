...
export class AufgabenDetailComponent implements OnInit {

  task : Task = new Task()
  departments : Array<Department> = []
  users : Array<User> = []

  public aufgabenForm = new FormGroup({
  ...
  ngOnInit(): void {
    if (this.route.snapshot.paramMap.get('id') !== null) {
      ...
      this.tasksService.getOne(id).subscribe(obj => {
        this.task = obj
        this.aufgabenForm = this.formBuilder.group(this.task)

        this.aufgabenForm.controls.members.setValue(this.task.members)
      })
    }
